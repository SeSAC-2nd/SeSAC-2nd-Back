const { User, Order, Post, ProductImage, Seller, Category, Wishlist, Address } = require('../../models/index');
  ...
  try {
    ...
    if (seller) {
      ...
      sellerOrders = await Order.findAll({
        attributes: ['allOrderId', 'invoiceNumber'],
        where: { sellerId },
        include: [{
          model: Post,
          attributes: ['postTitle', 'productPrice'],
...
exports.updateOrderInvoiceNumber = async (req, res) => {
  try {
    ...
    if (!orderId || !invoiceNumber) {
      return res.status(400).json({ error: '주문 ID 혹은 송장 번호가 필요합니다.' });
    }
    ...
    // 데이터 업데이트
    await order.update({ invoiceNumber });

    // 성공 응답
    res.status(200).json({ message: '송장 번호가 성공적으로 등록되었습니다.', invoiceNumber });
...
exports.getAddressPage = async (req, res) => {
  try {
    ...
    address = await Address.findAll({
      attributes: ['addName', 'zipCode', 'address', 'detailedAddress', 'phoneNum', 'isDefault', 'receiver'],
      where: { userId }
    });
